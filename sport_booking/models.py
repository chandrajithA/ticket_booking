...
class Stadium(models.Model):
    ...
    city = models.ForeignKey(City, on_delete=models.CASCADE, related_name="stadiums", null="False", blank="False")
    seats_count = models.IntegerField(null=True, blank=True, help_text="leave empty when adding. Automatically update flight seat count.")
...
def movie_poster_image_upload_path(instance, filename):
    ...
    id = instance.pk
    return f'sports_pester_images/{name}_ID_{id}/{filename}'
...
class StadiumSeat(models.Model):
    ...
    stadium = models.ForeignKey(Stadium, on_delete=models.CASCADE, null=False, blank=False)
    seat_number = models.CharField(max_length=10, null=False, blank=False) 
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.stadium} - Seat {self.seat_number}"
...
class SportsEvent(models.Model):
    sport = models.ForeignKey(Sport, on_delete=models.CASCADE, related_name="events", null=False, blank=False)
    stadium = models.ForeignKey(Stadium, on_delete=models.CASCADE, related_name="events", null=False, blank=False)
    title=models.CharField(max_length=100, null=False, blank=False)
    team1_name=models.CharField(max_length=50, null=False, blank=False)
    team2_name=models.CharField(max_length=50, null=False, blank=False)
    ...
    sport_date = models.DateField(null=False, blank=False)
    sport_time = models.TimeField(null=False, blank=False)
    is_booking_closed = models.BooleanField(default=False)
    created_at = models.DateTimeField(auto_now_add=True)
    

    def __str__(self):
        return f"{self.sport.category} {self.title} | {self.sport_date} {self.sport_time}"
    


class SportsEventSeatAvailability(models.Model):
    ...
    event = models.ForeignKey(SportsEvent, on_delete=models.CASCADE,  related_name='seats', null=False, blank=False)
    seat = models.ForeignKey(StadiumSeat, on_delete=models.CASCADE, null=False, blank=False)
    ...
    def __str__(self):
        return f"{self.event} - {self.seat.seat_number} [{self.status}]"



class SportsEventBooking(models.Model):
    STATUS = (
        ('pending', 'Pending'),
        ('confirmed', 'Confirmed'),
        ('cancelled', 'Cancelled'),
    ...
    def __str__(self):
        return f"{self.booking_reference} | {self.user} | {self.event.sport.category} | {self.status}"


class SportsEventBookingSeat(models.Model):
    booking = models.ForeignKey(SportsEventBooking, on_delete=models.CASCADE, null=False, blank=False)
    seat = models.ForeignKey(StadiumSeat, on_delete=models.CASCADE, null=False, blank=False)
...
class Payment(models.Model):
    ...
    booking = models.OneToOneField(SportsEventBooking, on_delete=models.CASCADE, null=False, blank=False)
