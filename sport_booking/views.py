...
from sport_booking.utils.booking_cleanup import clean_expired_bookings
from .models import Payment
import razorpay
from django.views.decorators.csrf import csrf_exempt
from sport_booking.payment_utils import calculate_booking_amount
...
def sports_display(request):
    ...
    events = SportsEvent.objects.select_related('sport', 'stadium__city').order_by('sport_date')

    # FILTER OPTIONS
    sports_types = sorted(set(
        events.values_list("sport__category", flat=True)
    ))

    cities = sorted(set(
        events.values_list("stadium__city__name", flat=True)
    ))
    ...
    return render(request, "sport_booking/sports_display.html", context)
...
def filter_sports(request):

    data = json.loads(request.body)

    qs = SportsEvent.objects.select_related('sport', 'stadium__city').all().order_by('sport_date')

    # SEARCH (teams)
    if data.get("search"):
        qs = qs.filter(
            Q(title__icontains=data["search"]) |
            Q(team1_name__icontains=data["search"]) |
            Q(team2_name__icontains=data["search"])
        )

    if data.get("date"):
        qs = qs.filter(sport_date=data["date"])

    # CITY
    if data.get("city"):
        qs = qs.filter(stadium__city__name=data["city"])

    # SPORT TYPE
    if data.get("sport"):
        qs = qs.filter(sport__category=data["sport"])
    ...
    events = [{
        "id": e.id,
        "title": e.title,
        "team": f"{e.team1_name} vs {e.team2_name}",
        "poster": e.poster.url if e.poster else "",
        "sport": e.sport.category,
        "date": e.sport_date.strftime("%B %d, %Y"),
        "time": e.sport_time.strftime("%I:%M %p"),
        "stadium": e.stadium.name,
        "city": e.stadium.city.name,
        "price": e.seat_price,
    } for e in qs]
...
def initiate_booking(request, event_id):     
    ...
    if request.method == "POST":
        ...
        event = SportsEvent.objects.filter(id=event_id, is_booking_closed=False).first()
        
        if event:

            seats = SportsEventSeatAvailability.objects.filter(
                event=event,
                status='available',
                is_booked=False
            )
            ...
            if not seats:
                ...
                return redirect("sport_booking:sports_display")
            ...
            booking = SportsEventBooking.objects.create(
                user=request.user,
                event=event,
            ...
            return JsonResponse({
                    "success": True,
                    "redirect_url": f"/sports/booking-details/{booking.id}/"
                })

    else:
        messages.info(request, "Sports event not found.")
        return redirect('sport_booking:sports_display')   
...
def booking_details(request, current_booking_id):   
    ...
    if request.session.pop('just_created_booking', False):
        ...
        booking = SportsEventBooking.objects.filter(id=current_booking_id,user=request.user, status='pending').select_related('event').first()
        if not booking:
            return redirect('sport_booking:sports_display')
        ...
        return render(request, 'sport_booking/booking_details.html', {
            'booking': booking,
        ...
        booking = SportsEventBooking.objects.filter(
...
def create_payment_order(request, booking_id):
    ...
    with transaction.atomic(): 
        ...
        available_seats = SportsEventSeatAvailability.objects.select_for_update().filter(
            event=booking.event,
            status='available',
            is_booked=False,
        ).order_by('seat__seat_number')[:ticket_count]
        ...
        for seat in available_seats:
            ...
            SportsEventBookingSeat.objects.create(
                booking=booking,
                seat=seat.seat,
...
def verify_payment(request):
    ...
    # 🔒 lock seats permanently
    SportsEventSeatAvailability.objects.filter(
        event=booking.event,
...
def expire_booking(request, booking_id):
    ...
    if booking and booking.expires_at <= timezone.now():
        ...
        SportsEventBookingSeat.objects.filter(
            booking=booking
        ).delete()
...
def payment_success(request, booking_id):
    ...
    booking = SportsEventBooking.objects.filter(
        ...
        user=request.user,
        status='confirmed'
    ).select_related('event__sport','event__stadium__city').first()

    if not booking:
        messages.error(request, "Error in booking.")
        return redirect('booking_app:index_page')
    
    booking_seats = SportsEventBookingSeat.objects.filter(
        booking=booking
    ).select_related('seat')

    return render(request, "sport_booking/payment_success.html", {
        "booking": booking,
...
def payment_failed(request, booking_id):
    ...
    return render(request, "sport_booking/payment_failed.html", {
        "booking": booking
    })