...
from .models import SportsEvent, SportsEventSeatAvailability, Stadium, StadiumSeat
...
@receiver(post_save, sender=SportsEvent)
def create_seats_for_sportevent(sender, instance, created, **kwargs):
    if created:
        if not SportsEventSeatAvailability.objects.filter(event=instance).exists():
...
def create_sport_event_seat_availability(event):
    ...
    SportsEventSeatAvailability.objects.bulk_create(seat_objects)
