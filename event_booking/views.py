...
from event_booking.utils.booking_cleanup import clean_expired_bookings
from .models import Payment
import razorpay
from django.views.decorators.csrf import csrf_exempt
from event_booking.payment_utils import calculate_booking_amount
...
def events_display(request):
    ...
    events = Event.objects.select_related('event', 'stadium__city').order_by('event_date')

    # FILTER OPTIONS
    event_types = sorted(set(
        events.values_list("event__category", flat=True)
    ))

    languages = sorted(set(
        events.values_list("language", flat=True)
    ))
    ...
    return render(request, "event_booking/events_display.html", context)
...
def filter_events(request):
    ...
    qs = Event.objects.select_related('event', 'stadium__city').all().order_by('event_date')
    ...
    # SPORT TYPE
    if data.get("event"):
        qs = qs.filter(event__category=data["event"])
    ...
    events = [{
        ...
        "sport": e.event.category,
        "language": e.language,
        "date": e.event_date.strftime("%B %d, %Y"),
        "time": e.event_time.strftime("%I:%M %p"),
    ...
    if request.method == "POST":
        ...
        if event:
            ...
            if not seats:
                ...
                return redirect("event_booking:events_display")
            ...
            return JsonResponse({
                    "success": True,
                    "redirect_url": f"/events/booking-details/{booking.id}/"
                })
...
def booking_details(request, current_booking_id):   
    ...
    if request.session.pop('just_created_booking', False):
        ...
        return render(request, 'event_booking/booking_details.html', {
            'booking': booking,
...
def create_payment_order(request, booking_id):
    ...
    with transaction.atomic(): 
        ...
        available_seats = EventSeatAvailability.objects.select_for_update().filter(
            event=booking.event,
            status='available',
            is_booked=False,
...
def payment_success(request, booking_id):
    ...
    booking = EventBooking.objects.filter(
        ...
        user=request.user,
        status='confirmed'
    ).select_related('event__event','event__stadium__city').first()
    ...
    return render(request, "event_booking/payment_success.html", {
        "booking": booking,
...
def payment_failed(request, booking_id):
    ...
    return render(request, "event_booking/payment_failed.html", {
        "booking": booking
    })