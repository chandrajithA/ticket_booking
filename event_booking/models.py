...
def event_poster_image_upload_path(instance, filename):
    from django.utils.text import slugify
    name = slugify(instance.title)
    id = instance.pk
    return f'event_pester_images/{name}_ID_{id}/{filename}'
...
class Event(models.Model):
    event = models.ForeignKey(EventType, on_delete=models.CASCADE, related_name="events", null=False, blank=False)
    ...
    poster = models.ImageField(upload_to=event_poster_image_upload_path, null=False, blank=False)
    ...
    def __str__(self):
        return f"{self.event.category} {self.title} | {self.event_date} {self.event_time}"
...
class EventSeatAvailability(models.Model):
    ...
    event = models.ForeignKey(Event, on_delete=models.CASCADE,  related_name='seats', null=False, blank=False)
...
class EventBooking(models.Model):
    ...
    def __str__(self):
        return f"{self.booking_reference} | {self.user} | {self.event.event.category} | {self.status}"
